...
export default class BackingTrackSelector {
  x: number
  y: number
  w: number
  h: number
    ...
    for (let row = 0; row < this.rows; row++) {
      ...
      for (let col = 0; col < this.cols; col++) {
        ...
        this.keyLocations.push({
          name: this.backingTracks[row * this.cols + col],
          x1: x_offset,
  ...
  show(p5: p5Types) {
    ...
    p5.textSize(12)
    const title = 'Backing Tracks (Jam Tracks @ youtube.com)'
    ...
    for (let row = 0; row < this.rows; row++) {
      ...
      for (let col = 0; col < this.cols; col++) {
        ...
        if (this.backingTracks[row * this.cols + col] == this.selected) {
        ...
        const text = this.backingTracks[row * this.cols + col]
        p5.text(
          text,
